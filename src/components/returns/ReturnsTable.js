...
function ReturnsTable() {
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to DELETE this transaction?')) {
      ...
      try {
        const result = await deleteReturnsTransaction(id);
        if (result.success) {
          // Show success alert
          setAlert({
            show: true,
  ...
  const handleCreateReturns = async (itemData) => {
    try {
      const result = await createReturnsTransaction(itemData);
      ...
      if (result.success) {
        ...
        setAlert({
          show: true,
          message: 'Returns transaction created successfully!',
          type: 'success'
        });
  ...
  const handleEditReturns = async (itemData) => {
    try {
      const result = await updateReturnsTransaction(itemData);
      if (result.success) {
        setShowEditModal(false);
        
        setAlert({
          show: true,
          message: 'Returns transaction updated successfully!',
          type: 'success'
        });
        setTimeout(() => {
          setAlert({ show: false, message: '', type: '' });
  ...
  const handleApproveReturns = async (itemData) => {
    try {
      const result = await approveReturnsTransaction(itemData);
      if (result.success) {
        setShowViewModal(false);
        
        setAlert({
          show: true,
          message: 'Returns transaction approved successfully!',
          type: 'success'
        });
        setTimeout(() => {
          setAlert({ show: false, message: '', type: '' });
    ...
    <div className="flex flex-col h-screen">
      ...
      <div className="flex-1 overflow-auto mt-2">
        ...
        <ViewReturnsModal
          ...
          onApprove={handleApproveReturns}
          id={selectedItem?.id}
        />
