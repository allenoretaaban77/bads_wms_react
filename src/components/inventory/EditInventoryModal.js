...
const EditInventoryModal = ({ selectedItem, showEditModal, setShowEditModal, onSave }) => {
  ...
  const validateForm = () => {
    ...
    if (!formData.cost_per_unit || formData.cost_per_unit === '') {
    ...
    if (!formData.initial_qty || formData.initial_qty === '') {
    ...
    if (!formData.current_qty || formData.current_qty === '') {
    ...
    if (!formData.reorder_level || formData.reorder_level === '') {
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={handleChange}
                  placeholder="0"
                  readOnly={userData?.position_id !== 1}
                  className={`w-full px-2 py-1.5 text-sm border rounded-custom focus:outline-none focus:ring-2 focus:ring-button focus:border-transparent ${
                    errors.initial_qty ? 'border-red-300 focus:ring-red-500' : 'border-gray-300'
                  } ${userData?.position_id !== 1 ? 'bg-gray-100 cursor-not-allowed' : ''}`}
                  required
                />
                {errors.initial_qty && (
